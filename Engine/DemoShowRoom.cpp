...
void DemoShowRoom::CreateTextures()
{
	...
	RENDERER(mCenterObj)->Texture = mCrateTex.get();
	...
	XMStoreFloat4x4(&RENDERER(mLWallObj)->TexTransform, mLWallObj->transform->LocalToWorldMatrix());
...
void DemoShowRoom::CreateObjects()
{
	...
	mCenterObj->transform->Scale(XMFLOAT3(2.5, 2.5, 2.5));
...
void DemoShowRoom::CreateMaterials()
{
	...
	floorMat.Ambient = XMFLOAT4(0.13, 0.2, 0.2, 1);
